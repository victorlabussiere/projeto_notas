...
<main class="h-full w-full flex flex-col py-10">
    <section>
        <form method="POST" action="/note" class=' flex flex-col gap-4 w-full'>
            ...
            <header class='flex gap-10 w-full items-center justify-end'>
                ...
                <input type="submit" value="Editar Nota" class='flex items-center py-2 px-3 gap-2 bg-lime-700 text-white font-semibold rounded-lg duration-150 hover:bg-lime-600 cursor-pointer'>
            </header>

            <label for="text" class="flex flex-col gap-1 text-lime-700">Texto da Nota
                <textarea type="text" name="body" id="text" placeholder="Exemplo de texto.." value='<?= $note['body'] ?>' class='p-2 w-full h-44 text-lime-600 rounded-md duration-100 transition-all shadow-md hover:shadow-lg'> <?= $note['body'] ?> </textarea>
